...
export class Relics {
  ...
  offer() {
    ...
    this.onPause(true);
    const box = this.modal.show(`<h2>Oferenda do Castelo</h2><div class="sub">Vesper: Escolha uma relíquia. Ela fica com você até o fim do mandato.</div>
  ...
  list() {
    ...
    this.modal.show(`<h2>Relíquias do mandato</h2><div class="sub">Uma nova escolha a cada ${RELIC_EVERY} noites</div>
      ${rows || `<p class="muted" style="text-align:center">Nenhuma ainda. A primeira oferenda chega na noite ${RELIC_EVERY}.</p>`}`);
  }
}
